...
const Button: FC<IProps & ButtonHTMLAttributes<HTMLButtonElement>> = ({
  ...
  const btnVariant =
    variant === "fill-primary"
      ? "bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-300 dark:bg-blue-700 dark:hover:bg-blue-800 dark:focus:ring-blue-800 disabled:bg-blue-400 disabled:dark:bg-blue-900 disabled:cursor-wait"
      : variant === "fill-danger"
  ...
  return (
    <button
      {...props}
      className={`rounded-lg text-sm font-medium outline-none transition duration-200 ease-in focus:ring-4 md:text-base ${btnSize} ${btnVariant}`}
    >
      {children}
    </button>
  );
